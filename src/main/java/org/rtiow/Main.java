...
public class Main {
    ...
    public Vec3 color(Ray r, HittableList world, int depth) {
        ...
        if (world.hit(r, 0.001F, Float.MAX_VALUE, rec)) {
            ...
            return (new Vec3(1.0F, 1.0F, 1.0F)).mult(1.0F-t).add((new Vec3(0.5F, 0.7F, 1.0F).mult(t)));
        }
    }
    public List<Hittable> randomScene() {
        ...
        for (int a = -11; a < 11; a++) {
            for (int b = -11; b < 11; b++) {
                ...
                if (center.sub(new Vec3(4F, 0.2F, 0F)).length() > 0.9F) {
                    if (choose_mat < 0.8F) {
    ...
    public static void main(String[] args) {
        ...
        HittableList world = new HittableList(new Main().randomScene(), 5);
        ...
        for (int j = ny-1; j >= 0; j--) {
            for (int i = 0; i < nx; i++) {
                ...
                for (int s = 0; s < ns; s++) {
                    ...
                    col = col.add(new Main().color(r, world, 0));
                }
                col = col.divide((float)ns);
